...
class CreateStreamHandler(webapp2.RequestHandler):
	def post(self):
		...
		if str(self.request.get("streamName")) == '':
			...
			stream.avatar = urllib.urlopen(coverURL).read()
...
class ViewAllStreams(webapp2.RequestHandler):
	def get(self):
		...
		stream_query = Stream.query().order(-Stream.dateCreated)
		output = {}
		for xx in stream_query:
			print xx
			strID = xx.streamName
			strURL = xx.coverURL
			output[strID] = strURL
			if strURL != '':
				f = (urllib.urlopen(strURL).read())
				b = bytearray(f)
				print "here is the bytearray"
				print b
		print output
...
class AndroidViewAllStreams(webapp2.RequestHandler):
	def get(self):
		...
		strName = self.request.get('strName')
		...
		if pos != '':
			...
			for s in stream_query:
				pos -= 1
				targetURL = s.coverURL
				targetAvatar = s.avatar
				if pos == -1:
					break

			if pos == -1:
				self.response.write(targetAvatar)
			...
			for s in stream_query:
				strName -= 1
				targetName = s.streamName
				if strName == -1:
					break

			if strName == -1:
				self.response.write(targetName)


class AndroidViewSingleStream(webapp2.RequestHandler):
	def get(self):
		pos = self.request.get('pos')
		strName = self.request.get('strName')
		pos = int(pos)
		...
		targetAvatar = None
		for pic in target_stream.photos:
			pos -= 1
			targetAvatar = pic.avatar
			if pos == -1:
				break
...
class AndroidUpload(webapp2.RequestHandler):
	def post(self):
		strName = (str)(self.request.get('strName'))
		...
		owner = targetStream.creator

		user_photo = Image(user=owner,blob_key = files.blobstore.get_blob_key(blob), avatar = blob)
...
class GeoViewHandler(webapp2.RequestHandler):
	def get(self):
		...
		for photo in photoList:
			#photoKeyList.append(str(photo.blob_key))
			photoTimeDict[str(photo.blob_key)]=str(photo.date)

		print "here is dumped photoKeyList"
		print photoTimeDict
	...
	def handle_upload(self, stream):
		...
		for name, fieldStorage in self.request.POST.items():
			...
			if self.validate(result):
				...
				'''
				time.sleep(3)
				if PhotoUploadHandler.visited is False:
					PhotoUploadHandler.visited = True
					for x in xrange(PhotoUploadHandler.keyIndex):
						print 'show me the keys'
						print PhotoUploadHandler.keys[x]
					q = Stream.query()
					for s in q:
						if str(s.key.id()) == stream:
							s.photos = s.photos
							for i in xrange(PhotoUploadHandler.keyIndex):
								s.photos.append(PhotoUploadHandler.keys[i])
							s.dateCreated=datetime.datetime.now()
							print 'before put size is ' + str(len(s.photos))
							s.put()
							print 'after put size is ' + str(len(s.photos))
					print 'totalUsedIndex is ' + str(PhotoUploadHandler.keyIndex)
					PhotoUploadHandler.keys = [0] * 10000
					PhotoUploadHandler.keyIndex = 0
					PhotoUploadHandler.visited = False
				else:
					pass
				'''
		...
		try:
			...
			for stream in q:
				if str(stream.key.id()) == self.request.get('stream'):
					print 'this is the correct blobkey: '
					print files.blobstore.get_blob_key(blob)
					user_photo = Image(user=users.get_current_user().user_id(),blob_key = files.blobstore.get_blob_key(blob), avatar = blob)
					user_photo.avatar = urllib.urlopen(images.get_serving_url(files.blobstore.get_blob_key(blob))).read()
					stream.photos.append(user_photo)
					stream.dateCreated=datetime.datetime.now()
					print 'put in'
					try:
...
application = webapp2.WSGIApplication([
	...
	('/androidViewAllStreams', AndroidViewAllStreams),
	('/androidViewSingleStream', AndroidViewSingleStream),
